...
export async function POST(
  ...
  try {
    ...
    // Client signature - 90% smaller, moved right to avoid covering label, positioned closer to line
    const signatureY = 335; // Just above the signature line
    const signatureX = col1X + 35; // Move right to avoid "Signature" label
    const signatureWidth = 135; // 90% of 150
    const signatureHeight = 36; // 90% of 40
    ...
    const dateY = 285; // Position closer to date line
    secondPage.drawText(customerSignDate, {
      x: col1X + 35, // Align with signature
      y: dateY,
      size: 10,
      font,
      color: rgb(0, 0, 0),
    ...
    const contractData = updateResult.rows[0];
